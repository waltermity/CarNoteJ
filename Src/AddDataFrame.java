...
public class AddDataFrame extends JFrame {
  ...
  /**
   * @param jarmu ShowFrame-ből érkezünk ide, paraméterként a kiválasztott járművet kapja meg
   *     TextFieldek kitöltésével vihetőek be az adatok, hozzáad gombbal hozzáadható a rekord Vissza
   *     gomb lenyomásával visszakerülünk a ShowFrame-be (Jármű adatait megjelenítő ablak)
   */
  public AddDataFrame(Jarmu jarmu) {
    ...
    // Hozzáadás gombra action rendelés, addAdat fgv, paramétereként a Textfield-ek értékeit adjuk,
    // ahol kell típus illesztés/konvertálással
        ...
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            try {
              jarmu.addAdat(
                  jtfkiadas.getText(),
                  Integer.parseInt(jtfosszeg.getText()),
                  Integer.parseInt(jtfkmallas.getText()),
                  jtfcomment.getText());
              setVisible(false);
              ShowFrame sh = new ShowFrame(jarmu);
              sh.setVisible(true);
